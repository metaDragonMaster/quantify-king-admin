				...
				<template #default="{ row }">
					<div class="table-expand">
						...
						<div class="flex">
							...
							<dl>
								<dt>操作</dt>
								<dd @click="openDialogReChild(row,'s1')"><u>查看当前好友</u>
								</dd>
								<dd @click="openDialogReChild(row,'s2')"><u>查看当前好友</u></dd>
								<dd @click="openDialogReChild(row,'s3')"><u>查看当前好友</u></dd>
							</dl>
						</div>
					</div>
				</template>
...
import { ref, defineExpose, reactive,  computed,nextTick, defineProps,defineEmits } from "vue";
	...
	deepData.map((item) => {
		...
		item.end_time =
			Number(item.end_time) > 0
				...
				: "未领取";
		item._freeze = item.freeze ? "未冻结" : "已冻结";
		item._released =
